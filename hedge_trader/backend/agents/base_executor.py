...
class BaseExecutor:
    ...
    async def _execute(self, itm: dict, price: float):
        ...
        _tp_mult     = float(self._cfg("partial_tp_multiplier") or 2.0)
        await self._log(
            f"FUTURES FILLED: {side} {qty} BTC @ {fut_px:.2f}  "
            f"Partial trigger @ {self.partial_trigger_price:.2f}  "
            f"(entry {fut_px:.2f} + premium {self.hedge_premium_paid:.2f} × {_tp_mult}x / qty {qty})  "
        ...
        await self._log_session_event("entry_snapshot", {
            ...
            "targets": {
                ...
                "partial_mode":          f"premium_x{float(_c('partial_tp_multiplier') or 2.0):.1f}",
